...
-- ---------------------------------------------------------------------------
-- VARIABLES NEARBY (déclarées ici pour être visibles dans tout le fichier)
-- ---------------------------------------------------------------------------
local nearbyEnemies = {}
...
local function InitDB()
    ...
    if type(NoMercyListDB.settings.nearby.showListed) ~= "boolean" then
        NoMercyListDB.settings.nearby.showListed = true
    end
    if type(NoMercyListDB.settings.nearby.showOthers) ~= "boolean" then
        NoMercyListDB.settings.nearby.showOthers = true
    end
    if type(NoMercyListDB.settings.nearby.visible) ~= "boolean" then
        NoMercyListDB.settings.nearby.visible = true  -- visible par défaut au premier lancement
    end

    -- Ennemis à portée (non listés)
    if type(NoMercyListDB.settings.nearbyOthers) ~= "table" then
        NoMercyListDB.settings.nearbyOthers = {}
    end
    if type(NoMercyListDB.settings.nearbyOthers.maxEntries) ~= "number" then
        NoMercyListDB.settings.nearbyOthers.maxEntries = 10
    end
    if type(NoMercyListDB.settings.nearbyOthers.expireTime) ~= "number" then
        NoMercyListDB.settings.nearbyOthers.expireTime = 30
    end
    if type(NoMercyListDB.settings.nearbyOthers.scale) ~= "number" then
        NoMercyListDB.settings.nearbyOthers.scale = 1.0
    end
    if type(NoMercyListDB.settings.nearbyOthers.opacity) ~= "number" then
        NoMercyListDB.settings.nearbyOthers.opacity = 0.8
    end

    -- Bouton minimap
    if type(NoMercyListDB.settings.minimapEnabled) ~= "boolean" then
        NoMercyListDB.settings.minimapEnabled = true
    end
...
local function FormatLastSeen(timestamp)
    ...
    local now = time()
    if not now then return L["TIME_NEVER"] end
    ...
    elseif elapsed < 3600 then
        return string.format(L["TIME_MIN"], math.floor(elapsed / 60))
    elseif elapsed < 86400 then
        return string.format(L["TIME_HOUR"], math.floor(elapsed / 3600))
    else
        return string.format(L["TIME_DAY"], math.floor(elapsed / 86400))
    end
end
...
eventFrame:SetScript("OnEvent", function(self, event, ...)
    ...
    elseif event == "PLAYER_LOGIN" then
        ...
        -- Restaure la visibilité du tableau nearby depuis la dernière session
        if NoMercyListDB.settings.nearby.visible and nearbyFrame then
            nearbyFrame:Show()
            RefreshNearbyFrame()
        end
...
TriggerAlert = function(playerName, unitToken)
    ...
    if playerData then
        ...
        if NoMercyListDB.settings.text.showRatio then
            ...
            table.insert(textParts, L["ALERT_RATIO"] .. ratio)
        end
...
function NoMercyList_ScanNearbyPlayers()
        ...
        if UnitExists(unit) and UnitIsPlayer(unit) and UnitIsEnemy("player", unit) then
            ...
            if name and not detected[name] then
                ...
                if SafePlayerInList(name) then
                    ...
                    if playerData then
                        UpdateNearbyEnemy(name, playerData, unit)
                    end
                else
                    -- Ennemi hostile non listé → deuxième tableau
                    UpdateNearbyOther(name, unit)
                end
            end
        end
    end
    ...
    for i = 1, numChildren do
        ...
        if frameName and frameName:find("NamePlate") then
...
local NEARBY_FADE_DURATION = 5  -- Durée du fade out (secondes) avant suppression définitive

-- Table 1 : joueurs DE la liste détectés à portée
-- Table 2 : joueurs HORS liste détectés à portée (déjà déclarés en haut du fichier)
...
local function CleanExpiredNearbyEnemies()
    local now = GetTime()
    local expireTime = NoMercyListDB.settings.nearby.expireTime or 30

    for name, data in pairs(nearbyEnemies) do
        -- Supprime seulement après la fin du fade (expireTime + NEARBY_FADE_DURATION)
        if (now - data.timestamp) > (expireTime + NEARBY_FADE_DURATION) then
...
local function GetSortedNearbyEnemies()
    ...
    for name, data in pairs(nearbyEnemies) do
        ...
        if age <= expireTime then
            table.insert(active, {name = name, data = data, alpha = 1.0})
        else
            local fadeAlpha = math.max(0, 1.0 - (age - expireTime) / NEARBY_FADE_DURATION)
            table.insert(fading, {name = name, data = data, alpha = fadeAlpha})
        end
    end

    table.sort(active, function(a, b) return a.data.timestamp > b.data.timestamp end)
    table.sort(fading, function(a, b) return a.alpha > b.alpha end)

    while #active > maxEntries do table.remove(active) end
    for _, entry in ipairs(fading) do table.insert(active, entry) end

    return active
end

-- ---------------------------------------------------------------------------
-- SYSTÈME "ENNEMIS À PORTÉE" HORS LISTE (v2.0)
-- ---------------------------------------------------------------------------

UpdateNearbyOther = function(playerName, unitID)
    if not playerName or playerName == PLAYER_NAME then return end
    local level, classToken = TryGetPlayerInfo(unitID)
    nearbyOthers[playerName] = {
        timestamp  = GetTime(),
        level      = level or 0,
        classToken = classToken or "",
        unitID     = unitID,
...
local function CleanExpiredNearbyOthers()
    local now = GetTime()
    local expireTime = NoMercyListDB.settings.nearbyOthers.expireTime or 30
    for name, data in pairs(nearbyOthers) do
        if (now - data.timestamp) > (expireTime + NEARBY_FADE_DURATION) then
            nearbyOthers[name] = nil
        end
    end
end

local function GetSortedNearbyOthers()
    CleanExpiredNearbyOthers()

    local now = GetTime()
    local expireTime = NoMercyListDB.settings.nearbyOthers.expireTime or 30
    local maxEntries = NoMercyListDB.settings.nearbyOthers.maxEntries or 10
...
function NoMercyList_BuildUI()
    ...
    mainFrame:Hide()
    mainFrame.TitleText:SetText("No Mercy List v1.5.2")
    ...
    mainFrame.headers = {}
    mainFrame.headers.name      = MakeHeader(L["COL_NAME"],    COL.name,      120, "LEFT")
    mainFrame.headers.level     = MakeHeader(L["COL_LEVEL"],   COL.level,      50, "CENTER")
    mainFrame.headers.class     = MakeHeader(L["COL_CLASS"],   COL.class,     120, "LEFT")
    mainFrame.headers.guild     = MakeHeader(L["COL_GUILD"],   COL.guild,     100, "LEFT")
    mainFrame.headers.deaths    = MakeHeader(L["COL_DEATHS"],  COL.deaths,     55, "CENTER")
    mainFrame.headers.kills     = MakeHeader(L["COL_KILLS"],   COL.kills,      55, "CENTER")
    mainFrame.headers.ratio     = MakeHeader(L["COL_RATIO"],   COL.ratio,      55, "CENTER")
    mainFrame.headers.hateLevel = MakeHeader(L["COL_HATE"],    COL.hateLevel,  35, "CENTER")
    mainFrame.headers.lastSeen  = MakeHeader(L["COL_LASTSEEN"],COL.lastSeen,  160, "LEFT")
    ...
    addBtn:SetText(L["BTN_ADD_PLAYER"])
    ...
    addBtn:SetScript("OnEnter", function(self)
        ...
        GameTooltip:SetText(L["TIP_ADD_PLAYER"], 1, 0.82, 0)
        GameTooltip:AddLine(L["TIP_ADD_ENEMY"], 1, 1, 1)
        GameTooltip:AddLine(L["TIP_ADD_ENEMY2"], 0.4, 1, 0.4)
        GameTooltip:AddLine(L["TIP_ADD_NOTARGET"], 1, 1, 1)
        GameTooltip:AddLine(L["TIP_ADD_NOTARGET2"], 0.8, 0.8, 0.8)
        GameTooltip:Show()
    end)
    ...
    histBtn:SetScript("OnEnter", function(self)
        GameTooltip:SetOwner(self, "ANCHOR_TOP")
        GameTooltip:SetText(L["TIP_HISTORY"], 1, 0.82, 0)
        GameTooltip:AddLine(L["TIP_HISTORY2"], 1, 1, 1)
        GameTooltip:AddLine(L["TIP_HISTORY3"], 1, 1, 1)
        GameTooltip:Show()
    end)
    ...
    settingsBtn:SetScript("OnEnter", function(self)
        GameTooltip:SetOwner(self, "ANCHOR_TOP")
        GameTooltip:SetText(L["TIP_SETTINGS"], 1, 0.82, 0)
        GameTooltip:AddLine(L["TIP_SETTINGS2"], 1, 1, 1)
        GameTooltip:Show()
    end)
    ...
    radarBtn:SetScript("OnEnter", function(self)
        ...
        GameTooltip:SetOwner(self, "ANCHOR_TOP")
        GameTooltip:SetText(L["TIP_NEARBY"], 1, 0.82, 0)
        GameTooltip:AddLine(L["TIP_NEARBY2"], 1, 1, 1)
        GameTooltip:Show()
    end)
...
function NoMercyList_TryAddTarget()
    ...
    if UnitExists("target") and UnitIsPlayer("target") and UnitIsEnemy("player", "target") then
        ...
        if ok then
            -- Seede kills/deaths depuis l'historique des attaquants (si ajouté en combat)
            local hist = attackerHistory[tName]
            if hist then
                local p = NoMercyListDB.players[tName]
                if p then
                    if (hist.killedByMe or 0) > 0 then
                        p.kills = (p.kills or 0) + hist.killedByMe
                    end
                    if (hist.killedMe or 0) > 0 then
                        p.deaths = (p.deaths or 0) + hist.killedMe
                    end
                end
            end
            -- Retire du tableau hors-liste et force dans le tableau liste
            nearbyOthers[tName] = nil
            local p = NoMercyListDB.players[tName]
            if p then UpdateNearbyEnemy(tName, p, "target") end
            -- Ouvre la fenêtre si masquée
            if mainFrame and not mainFrame:IsShown() then
                mainFrame:Show()
            end
            NoMercyList_Refresh()
            -- Force refresh du nearby immédiatement
            ...
            print("|cff00ff00[No Mercy List]|r " .. result .. L["MSG_ADDED_TARGET"]
                  .. " " .. L["ALERT_LEVEL"] .. lvlStr .. " | " .. cls .. guildStr)
        elseif result == "doublon" then
            print("|cffff4444[No Mercy List]|r " .. (tName or "?") .. L["MSG_ALREADY_LISTED"])
        else
            print("|cffff4444[No Mercy List]|r " .. L["MSG_CANT_ADD"])
        end
    else
...
function NoMercyList_ShowPopup()
    if not popupFrame then
        ...
        pop.TitleText:SetText(L["WIN_ADD"])
        ...
        btnAdd:SetScript("OnClick", function()
            ...
            if ok then
                -- Seede kills/deaths depuis l'historique (si ajouté en combat)
                local hist = attackerHistory[result]
                if hist then
                    local p = NoMercyListDB.players[result]
                    if p then
                        if (hist.killedByMe or 0) > 0 then p.kills = (p.kills or 0) + hist.killedByMe end
                        if (hist.killedMe or 0) > 0 then p.deaths = (p.deaths or 0) + hist.killedMe end
                    end
                end
                nearbyOthers[result] = nil
                local p = NoMercyListDB.players[result]
                if p then UpdateNearbyEnemy(result, p, nil) end
                NoMercyList_Refresh()
                if nearbyFrame and nearbyFrame:IsShown() then RefreshNearbyFrame() end
                msg:SetText("|cff00ff00" .. L["MSG_ADDED"] .. result .. "|r")
                eb:SetText("")
                C_Timer.After(0.9, function() pop:Hide() end)
            elseif result == "doublon" then
                msg:SetText("|cffff4444" .. L["MSG_ALREADY_SHORT"] .. "|r")
            else
                msg:SetText("|cffff4444" .. L["MSG_INVALID_NAME"] .. "|r")
            end
        end)

        local btnCancel = CreateFrame("Button", nil, pop, "UIPanelButtonTemplate")
        btnCancel:SetSize(110, 24)
        btnCancel:SetText(L["BTN_CANCEL"])
...
function NoMercyList_BuildNearbyFrame()
    ...
    nearbyFrame:SetSize(200, 100)
    nearbyFrame:SetScale(NoMercyListDB.settings.nearby.scale or 1.0)
    ...
    nearbyFrame.title:SetText(L["NEARBY_LISTED"])
    ...
    closeBtn:SetScript("OnClick", function()
        NoMercyListDB.settings.nearby.visible = false
        nearbyFrame:Hide()
    end)
    ...
    -- Pré-créer 10 boutons pour la section "Ennemis à portée" (hors liste)
    nearbyFrame.othersButtons = {}
    for i = 1, 10 do
        local btn = CreateFrame("Button", "NMLNearbyOtherButton"..i, nearbyFrame, "SecureActionButtonTemplate")
        btn:SetSize(180, 20)
        btn:SetPoint("TOP", 0, -25)  -- position placeholder, mise à jour dans Refresh
        ...
        btn.classIcon = btn:CreateTexture(nil, "ARTWORK")
        btn.classIcon:SetSize(16, 16)
        btn.classIcon:SetPoint("LEFT", 2, 0)

        btn.nameText = btn:CreateFontString(nil, "OVERLAY", "GameFontNormalSmall")
        btn.nameText:SetPoint("LEFT", btn.classIcon, "RIGHT", 3, 0)
        btn.nameText:SetJustifyH("LEFT")

        btn.levelText = btn:CreateFontString(nil, "OVERLAY", "GameFontNormalSmall")
        btn.levelText:SetPoint("RIGHT", btn, "RIGHT", -4, 0)
        btn.levelText:SetJustifyH("RIGHT")
        btn.levelText:SetTextColor(0.7, 0.7, 0.7)
        ...
        btn:Hide()
        nearbyFrame.othersButtons[i] = btn
    end

    -- Séparateur entre les deux sections
    nearbyFrame.sectionSep = nearbyFrame:CreateTexture(nil, "ARTWORK")
    nearbyFrame.sectionSep:SetHeight(1)
    nearbyFrame.sectionSep:SetColorTexture(0.5, 0.5, 0.5, 0.5)
    nearbyFrame.sectionSep:Hide()

    -- Titre de la section "Ennemis à portée" (hors liste)
    nearbyFrame.sectionTitle = nearbyFrame:CreateFontString(nil, "OVERLAY", "GameFontNormalSmall")
    nearbyFrame.sectionTitle:SetText(L["NEARBY_OTHERS"])
    nearbyFrame.sectionTitle:SetTextColor(1, 0.5, 0.2)
    nearbyFrame.sectionTitle:Hide()
...
-- Toggle la fenêtre nearby (frame unifiée)
function NoMercyList_ToggleNearby()
    if not nearbyFrame then return end

    local showListed = NoMercyListDB.settings.nearby.showListed ~= false
    local showOthers = NoMercyListDB.settings.nearby.showOthers ~= false

    -- Si aucune section n'est activée, on ne montre rien
    if not showListed and not showOthers then
        NoMercyListDB.settings.nearby.visible = false
        nearbyFrame:Hide()
        return
    end
    ...
    if nearbyFrame:IsShown() then
        ...
        NoMercyListDB.settings.nearby.visible = true
        nearbyFrame:Show()
        RefreshNearbyFrame()
    end
end

-- Refresh unifié : section 1 (liste) + section 2 (hors liste) dans le même frame
RefreshNearbyFrame = function()
    if not nearbyFrame then return end

    local settings = NoMercyListDB.settings
    local showListed = settings.nearby.showListed ~= false   -- default true
    local showOthers = settings.nearby.showOthers ~= false   -- default true

    -- Cache tout d'abord
    for i = 1, 10 do
        nearbyFrame.buttons[i]:SetAlpha(1.0)
        nearbyFrame.buttons[i]:Hide()
    end
    for i = 1, 10 do
        nearbyFrame.othersButtons[i]:SetAlpha(1.0)
        nearbyFrame.othersButtons[i]:Hide()
    end
    nearbyFrame.sectionSep:Hide()
    nearbyFrame.sectionTitle:Hide()

    -- Met à jour le titre principal selon la(les) section(s) active(s)
    if showOthers and not showListed then
        nearbyFrame.title:SetText(L["NEARBY_OTHERS"])
        nearbyFrame.title:SetTextColor(1, 0.5, 0.2)
    else
        nearbyFrame.title:SetText(L["NEARBY_LISTED"])
        nearbyFrame.title:SetTextColor(1, 0.82, 0)
    end

    local currentY = -25   -- position Y de départ (sous le titre)
    local count1 = 0
    local count2 = 0

    -- ---- Section 1 : Mes ennemis à portée (dans la liste) ----
    if showListed then
        local sorted = GetSortedNearbyEnemies()
        count1 = math.min(#sorted, 10)
        ...
        for i = 1, count1 do
            ...
            btn.nameText:SetText("|cff" .. hex .. entry.name .. "|r")
            ...
            btn:ClearAllPoints()
            btn:SetPoint("TOP", nearbyFrame, "TOP", 0, startY - (i-1)*20)
            btn:SetAlpha(entry.alpha or 1.0)
            btn:Show()
        end
        currentY = currentY - count1 * 20
    end

    -- ---- Séparateur (seulement si les deux sections sont actives) ----
    if showListed and showOthers then
        nearbyFrame.sectionSep:ClearAllPoints()
        nearbyFrame.sectionSep:SetPoint("TOPLEFT",  nearbyFrame, "TOPLEFT",  5, currentY - 3)
        nearbyFrame.sectionSep:SetPoint("TOPRIGHT", nearbyFrame, "TOPRIGHT", -5, currentY - 3)
        nearbyFrame.sectionSep:Show()

        nearbyFrame.sectionTitle:ClearAllPoints()
        nearbyFrame.sectionTitle:SetPoint("TOP", nearbyFrame, "TOP", 0, currentY - 8)
        nearbyFrame.sectionTitle:Show()

        currentY = currentY - 16
    end

    -- ---- Section 2 : Ennemis à portée (hors liste) ----
    if showOthers then
        local sorted = GetSortedNearbyOthers()
        count2 = math.min(#sorted, 10)
        local startY = currentY
        for i = 1, count2 do
            local entry = sorted[i]
            local btn   = nearbyFrame.othersButtons[i]
            local data  = entry.data
            ...
            local hex = CLASS_COLOR[data.classToken] or "AAAAAA"
            btn.nameText:SetText("|cff" .. hex .. entry.name .. "|r")

            local lvl = data.level or 0
            btn.levelText:SetText(lvl > 0 and tostring(lvl) or "?")
        ...
        currentY = currentY - count2 * 20
    end

    -- Ajuste la hauteur du frame selon le contenu réel
    local frameHeight = math.abs(currentY) + 10
    nearbyFrame:SetHeight(math.max(frameHeight, 50))
end

-- Timer pour refresh auto (nearby 0.5s + tableau principal 3s)
local nearbyUpdateTimer = 0
local mainUpdateTimer   = 0
StartNearbyUpdates = function()
    local f = CreateFrame("Frame")
    f:SetScript("OnUpdate", function(self, elapsed)
        nearbyUpdateTimer = nearbyUpdateTimer + elapsed
        if nearbyUpdateTimer >= 0.5 then
            nearbyUpdateTimer = 0
            if nearbyFrame and nearbyFrame:IsShown() then
                RefreshNearbyFrame()
            end
        end
        mainUpdateTimer = mainUpdateTimer + elapsed
        if mainUpdateTimer >= 3 then
            mainUpdateTimer = 0
            if mainFrame and mainFrame:IsShown() then
                NoMercyList_Refresh()
            end
...
-- ---------------------------------------------------------------------------
-- LEGACY: fonction conservée vide pour compatibilité (frame fusionnée dans nearbyFrame)
-- ---------------------------------------------------------------------------

function NoMercyList_BuildNearbyOthersFrame()
    -- No-op : la section "Ennemis à portée" est maintenant intégrée dans nearbyFrame
end
...
function NoMercyList_BuildMinimap()
    ...
    local btn = CreateFrame("Button", "NoMercyListMinimapButton", UIParent)
    btn:SetSize(32, 32)
    ...
    -- Restaure position sauvegardée, ou position par défaut près de la minimap
    if NoMercyListDB.minimapPos then
        local pos = NoMercyListDB.minimapPos
        btn:SetPoint(pos[1], UIParent, pos[2], pos[3], pos[4])
    else
        btn:SetPoint("TOPRIGHT", UIParent, "TOPRIGHT", -210, -5)
    end
    ...
    btn:SetScript("OnDragStop", function(self)
        ...
        local point, _, relativePoint, x, y = self:GetPoint()
        NoMercyListDB.minimapPos = {point, relativePoint, x, y}
        -- Délai court pour ne pas déclencher OnClick après le drag
        C_Timer.After(0.05, function() self._dragging = false end)
    end)
    ...
    btn:SetScript("OnEnter", function(self)
        ...
        GameTooltip:AddLine(L["TIP_MINIMAP2"], 1, 1, 1)
        GameTooltip:AddLine(L["TIP_MINIMAP3"], 0.7, 0.7, 0.7)
        GameTooltip:Show()
    end)
    btn:SetScript("OnLeave", function() GameTooltip:Hide() end)

    -- Affiche ou masque selon le réglage
    if NoMercyListDB.settings.minimapEnabled then
        btn:Show()
    else
        btn:Hide()
    end
...
function NoMercyList_ShowAttackerHistory()
    if not historyFrame then
        ...
        hf:SetToplevel(true)
        hf.TitleText:SetText(L["WIN_HISTORY"])
        ...
        hf.emptyMsg:SetText(L["MSG_NO_ATTACKERS"])
...
function NoMercyList_RefreshAttackerHistory()
    ...
    for i, data in ipairs(attackers) do
        ...
        if not row then
            ...
            row.addBtn:SetText(L["BTN_ADD_TO_LIST"])
        ...
        if data.level and data.level > 0 then
            levelStr = L["HIST_LEVEL"] .. data.level .. ")"
        end
        ...
        local timeStr
        if elapsed < 60 then
            timeStr = string.format(L["TIME_SEC"], math.floor(elapsed))
        else
            timeStr = string.format(L["TIME_MIN"], math.floor(elapsed / 60))
        end
        ...
        if killedMe > 0 or killedByMe > 0 then
            outcomeStr = L["HIST_FIGHT"]
            if killedByMe > 0 then
                outcomeStr = outcomeStr .. "|cff00ff00" .. killedByMe .. " " .. L["HIST_KILL"] .. "|r"
            end
            if killedMe > 0 then
                if killedByMe > 0 then outcomeStr = outcomeStr .. " / " end
                outcomeStr = outcomeStr .. "|cffff4444" .. killedMe .. " " .. L["HIST_DEATH"] .. "|r"
            end
        ...
        row.addBtn:SetScript("OnClick", function()
            ...
            if ok then
                NoMercyList_Refresh()
                print("|cff00ff00[No Mercy List]|r " .. result .. L["MSG_ADDED_HIST"])
                -- Grise le bouton pour éviter les doublons
                row.addBtn:Disable()
                row.addBtn:SetText(L["BTN_ALREADY_ADDED"])
            elseif result == "doublon" then
                row.addBtn:Disable()
                row.addBtn:SetText(L["BTN_ALREADY_LISTED"])
            end
        end)
...
function NoMercyList_ShowSettings()
    if not settingsFrame then
        ...
        sf.title:SetText(L["WIN_SETTINGS"])
        ...
        scrollChild:SetHeight(2000)  -- Hauteur du contenu
        ...
        -- ====================================================================
        MakeSectionTitle(L["SET_SEC_BORDER"], yOffset)
        yOffset = yOffset - 35
        ...
        enableBorderCb.text:SetText(L["SET_BORDER_ENABLE"])
        ...
        colorLabel:SetText(L["SET_COLOR"])
        ...
        colorHelp:SetText(L["SET_CLICK_CHOOSE"])
        ...
        durationLabel:SetText(L["SET_BORDER_DUR"])
        ...
        pulseLabel:SetText(L["SET_BORDER_SPEED"])
        ...
        if lowText then lowText:SetText(L["SET_SPEED_FAST"]) end
        if highText then highText:SetText(L["SET_SPEED_SLOW"]) end
        ...
        testVisualBtn:SetText(L["BTN_TEST_VISUAL"])
        ...
        -- ====================================================================
        MakeSectionTitle(L["SET_SEC_TEXT"], yOffset)
        yOffset = yOffset - 35
        ...
        enableTextCb.text:SetText(L["SET_TEXT_ENABLE"])
        ...
        textColorLabel:SetText(L["SET_TEXT_COLOR"])
        ...
        textColorHelp:SetText(L["SET_CLICK_CHOOSE"])
        ...
        textDurationLabel:SetText(L["SET_TEXT_DUR"])
        ...
        posLabel:SetText(L["SET_POSITION"])
        
        yOffset = yOffset - 35
        
        local positions = {
            {value = "TOP",    text = L["SET_POS_TOP"]},
            {value = "CENTER", text = L["SET_POS_CENTER"]},
            {value = "BOTTOM", text = L["SET_POS_BOTTOM"]},
        }
        ...
        sizeLabel:SetText(L["SET_FONT_SIZE"])
        ...
        elemLabel:SetText(L["SET_ELEMENTS"])
        
        yOffset = yOffset - 35
        
        local textElements = {
            {key = "showClass",     label = L["SET_SHOW_CLASS"]},
            {key = "showRatio",     label = L["SET_SHOW_RATIO"]},
            {key = "showHateLevel", label = L["SET_SHOW_HATE"]},
        }
        ...
        testTextBtn:SetText(L["BTN_TEST_TEXT"])
        ...
        -- ====================================================================
        MakeSectionTitle(L["SET_SEC_SOUND"], yOffset)
        yOffset = yOffset - 35
        ...
        enableSoundCb.text:SetText(L["SET_SOUND_ENABLE"])
        ...
        soundLabel:SetText(L["SET_SOUND_CHOOSE"])
        ...
        local sounds = {
            ...
            {id = 11466, name = L["SOUND_ALARM"]},
            {id = 3081, name = L["SOUND_BELL"]},
            {id = 8068, name = "Gong"},
        ...
        UIDropDownMenu_SetText(soundDropdown, L["SET_SOUND_SELECT"])
        ...
        -- ====================================================================
        MakeSectionTitle(L["SET_SEC_COOLDOWN"], yOffset)
        yOffset = yOffset - 35
        ...
        cooldownLabel:SetText(L["SET_COOLDOWN"])
        ...
        -- ====================================================================
        MakeSectionTitle(L["SET_SEC_COLUMNS"], yOffset)
        yOffset = yOffset - 35
        ...
        columnsLabel:SetText(L["SET_COL_LABEL"])
        
        yOffset = yOffset - 35
        
        local columnsList = {
            {key = "level",     label = L["SET_COL_LEVEL"]},
            {key = "class",     label = L["SET_COL_CLASS"]},
            {key = "guild",     label = L["SET_COL_GUILD"]},
            {key = "deaths",    label = L["SET_COL_DEATHS"]},
            {key = "kills",     label = L["SET_COL_KILLS"]},
            {key = "ratio",     label = L["SET_COL_RATIO"]},
            {key = "hateLevel", label = L["SET_COL_HATE"]},
            {key = "lastSeen",  label = L["SET_COL_LASTSEEN"]},
            {key = "delete",    label = L["SET_COL_DELETE"]},
        }
        ...
        -- ====================================================================
        -- SECTION 6 : MES ENNEMIS À PORTÉE (joueurs de la liste)
        -- ====================================================================
        MakeSectionTitle(L["SET_SEC_NEARBY"], yOffset)
        yOffset = yOffset - 35

        -- Checkbox "Afficher cette section"
        local listedCb = CreateFrame("CheckButton", "NMLNearbyListedCheckbox", scrollChild, "UICheckButtonTemplate")
        listedCb:SetPoint("TOPLEFT", scrollChild, "TOPLEFT", 20, yOffset)
        listedCb.text = listedCb:CreateFontString(nil, "OVERLAY", "GameFontNormal")
        listedCb.text:SetPoint("LEFT", listedCb, "RIGHT", 5, 0)
        listedCb.text:SetText(L["SET_NEARBY_LISTED"])
        listedCb:SetChecked(NoMercyListDB.settings.nearby.showListed ~= false)
        listedCb:SetScript("OnClick", function(self)
            NoMercyListDB.settings.nearby.showListed = self:GetChecked()
            if nearbyFrame and nearbyFrame:IsShown() then
                RefreshNearbyFrame()
            end
        end)
        ...
        local nearbyMaxLabel = scrollChild:CreateFontString(nil, "OVERLAY", "GameFontNormal")
        nearbyMaxLabel:SetPoint("TOPLEFT", scrollChild, "TOPLEFT", 20, yOffset)
        nearbyMaxLabel:SetText(L["SET_MAX_ENTRIES"])

        yOffset = yOffset - 35
        local nearbyMaxSlider = CreateFrame("Slider", "NMLNearbyMaxSlider", scrollChild, "OptionsSliderTemplate")
        nearbyMaxSlider:SetPoint("TOPLEFT", scrollChild, "TOPLEFT", 30, yOffset)
        nearbyMaxSlider:SetWidth(200)
        nearbyMaxSlider:SetMinMaxValues(1, 20)
        nearbyMaxSlider:SetValueStep(1)
        nearbyMaxSlider:SetValue(NoMercyListDB.settings.nearby.maxEntries or 10)

        local lowText = _G[nearbyMaxSlider:GetName().."Low"]
        local highText = _G[nearbyMaxSlider:GetName().."High"]
        local valueText = _G[nearbyMaxSlider:GetName().."Text"]
        if lowText then lowText:SetText("1") end
        if highText then highText:SetText("20") end
        if valueText then valueText:SetText(tostring(math.floor(NoMercyListDB.settings.nearby.maxEntries or 10))) end

        nearbyMaxSlider:SetScript("OnValueChanged", function(self, value)
            value = math.floor(value)
            NoMercyListDB.settings.nearby.maxEntries = value
            local txt = _G[self:GetName().."Text"]
            if txt then txt:SetText(tostring(value)) end
        end)
        ...
        -- Durée d'expiration
        local nearbyExpireLabel = scrollChild:CreateFontString(nil, "OVERLAY", "GameFontNormal")
        nearbyExpireLabel:SetPoint("TOPLEFT", scrollChild, "TOPLEFT", 20, yOffset)
        nearbyExpireLabel:SetText(L["SET_EXPIRE_TIME"])

        yOffset = yOffset - 35
        local nearbyExpireSlider = CreateFrame("Slider", "NMLNearbyExpireSlider", scrollChild, "OptionsSliderTemplate")
        nearbyExpireSlider:SetPoint("TOPLEFT", scrollChild, "TOPLEFT", 30, yOffset)
        nearbyExpireSlider:SetWidth(200)
        nearbyExpireSlider:SetMinMaxValues(5, 120)
        nearbyExpireSlider:SetValueStep(5)
        nearbyExpireSlider:SetValue(NoMercyListDB.settings.nearby.expireTime or 30)

        local lowText = _G[nearbyExpireSlider:GetName().."Low"]
        local highText = _G[nearbyExpireSlider:GetName().."High"]
        local valueText = _G[nearbyExpireSlider:GetName().."Text"]
        if lowText then lowText:SetText("5s") end
        if highText then highText:SetText("120s") end
        if valueText then valueText:SetText(tostring(math.floor(NoMercyListDB.settings.nearby.expireTime or 30)).."s") end

        nearbyExpireSlider:SetScript("OnValueChanged", function(self, value)
            value = math.floor(value)
            NoMercyListDB.settings.nearby.expireTime = value
            local txt = _G[self:GetName().."Text"]
            if txt then txt:SetText(tostring(value).."s") end
        end)
        ...
        -- Échelle
        local nearbyScaleLabel = scrollChild:CreateFontString(nil, "OVERLAY", "GameFontNormal")
        nearbyScaleLabel:SetPoint("TOPLEFT", scrollChild, "TOPLEFT", 20, yOffset)
        nearbyScaleLabel:SetText(L["SET_SCALE"])

        yOffset = yOffset - 35
        local nearbyScaleSlider = CreateFrame("Slider", "NMLNearbyScaleSlider", scrollChild, "OptionsSliderTemplate")
        nearbyScaleSlider:SetPoint("TOPLEFT", scrollChild, "TOPLEFT", 30, yOffset)
        nearbyScaleSlider:SetWidth(200)
        nearbyScaleSlider:SetMinMaxValues(0.5, 2.0)
        nearbyScaleSlider:SetValueStep(0.1)
        nearbyScaleSlider:SetValue(NoMercyListDB.settings.nearby.scale or 1.0)

        local lowText = _G[nearbyScaleSlider:GetName().."Low"]
        local highText = _G[nearbyScaleSlider:GetName().."High"]
        local valueText = _G[nearbyScaleSlider:GetName().."Text"]
        if lowText then lowText:SetText("50%") end
        if highText then highText:SetText("200%") end
        if valueText then valueText:SetText(string.format("%.0f%%", (NoMercyListDB.settings.nearby.scale or 1.0) * 100)) end

        nearbyScaleSlider:SetScript("OnValueChanged", function(self, value)
            NoMercyListDB.settings.nearby.scale = value
            local txt = _G[self:GetName().."Text"]
            if txt then txt:SetText(string.format("%.0f%%", value * 100)) end
            -- Application immédiate sur la fenêtre nearby
            if nearbyFrame then
                nearbyFrame:SetScale(value)
            end
        end)
        ...
        -- Opacité du fond
        local nearbyOpacityLabel = scrollChild:CreateFontString(nil, "OVERLAY", "GameFontNormal")
        nearbyOpacityLabel:SetPoint("TOPLEFT", scrollChild, "TOPLEFT", 20, yOffset)
        nearbyOpacityLabel:SetText(L["SET_OPACITY"])

        yOffset = yOffset - 35
        local nearbyOpacitySlider = CreateFrame("Slider", "NMLNearbyOpacitySlider", scrollChild, "OptionsSliderTemplate")
        nearbyOpacitySlider:SetPoint("TOPLEFT", scrollChild, "TOPLEFT", 30, yOffset)
        nearbyOpacitySlider:SetWidth(200)
        nearbyOpacitySlider:SetMinMaxValues(0.1, 1.0)
        nearbyOpacitySlider:SetValueStep(0.05)
        nearbyOpacitySlider:SetValue(NoMercyListDB.settings.nearby.opacity or 0.8)

        local lowText = _G[nearbyOpacitySlider:GetName().."Low"]
        local highText = _G[nearbyOpacitySlider:GetName().."High"]
        local valueText = _G[nearbyOpacitySlider:GetName().."Text"]
        if lowText then lowText:SetText("10%") end
        if highText then highText:SetText("100%") end
        if valueText then valueText:SetText(tostring(math.floor((NoMercyListDB.settings.nearby.opacity or 0.8) * 100)).."%") end

        nearbyOpacitySlider:SetScript("OnValueChanged", function(self, value)
            NoMercyListDB.settings.nearby.opacity = value
            local txt = _G[self:GetName().."Text"]
            if txt then txt:SetText(tostring(math.floor(value * 100)).."%") end
            -- Application immédiate sur le fond de la fenêtre nearby
            if nearbyFrame and nearbyFrame.bg then
                nearbyFrame.bg:SetColorTexture(0, 0, 0, value)
            end
        end)
        ...
        -- ====================================================================
        -- SECTION 7 : ENNEMIS À PORTÉE (hors liste)
        -- ====================================================================
        MakeSectionTitle(L["SET_SEC_OTHERS"], yOffset)
        yOffset = yOffset - 35

        -- Checkbox "Afficher cette section"
        local othersCb = CreateFrame("CheckButton", "NMLNearbyOthersCheckbox", scrollChild, "UICheckButtonTemplate")
        othersCb:SetPoint("TOPLEFT", scrollChild, "TOPLEFT", 20, yOffset)
        othersCb.text = othersCb:CreateFontString(nil, "OVERLAY", "GameFontNormal")
        othersCb.text:SetPoint("LEFT", othersCb, "RIGHT", 5, 0)
        othersCb.text:SetText(L["SET_OTHERS_SHOW"])
        othersCb:SetChecked(NoMercyListDB.settings.nearby.showOthers ~= false)
        othersCb:SetScript("OnClick", function(self)
            NoMercyListDB.settings.nearby.showOthers = self:GetChecked()
            if nearbyFrame and nearbyFrame:IsShown() then
                RefreshNearbyFrame()
            end
        end)
        ...
        -- Nombre max d'entrées
        local othersMaxLabel = scrollChild:CreateFontString(nil, "OVERLAY", "GameFontNormal")
        othersMaxLabel:SetPoint("TOPLEFT", scrollChild, "TOPLEFT", 20, yOffset)
        othersMaxLabel:SetText(L["SET_MAX_ENTRIES"])

        yOffset = yOffset - 35
        local othersMaxSlider = CreateFrame("Slider", "NMLOthersMaxSlider", scrollChild, "OptionsSliderTemplate")
        othersMaxSlider:SetPoint("TOPLEFT", scrollChild, "TOPLEFT", 30, yOffset)
        othersMaxSlider:SetWidth(200)
        othersMaxSlider:SetMinMaxValues(1, 20)
        othersMaxSlider:SetValueStep(1)
        othersMaxSlider:SetValue(NoMercyListDB.settings.nearbyOthers.maxEntries or 10)

        local lowText = _G[othersMaxSlider:GetName().."Low"]
        local highText = _G[othersMaxSlider:GetName().."High"]
        local valueText = _G[othersMaxSlider:GetName().."Text"]
        if lowText then lowText:SetText("1") end
        if highText then highText:SetText("20") end
        if valueText then valueText:SetText(tostring(math.floor(NoMercyListDB.settings.nearbyOthers.maxEntries or 10))) end

        othersMaxSlider:SetScript("OnValueChanged", function(self, value)
            value = math.floor(value)
            NoMercyListDB.settings.nearbyOthers.maxEntries = value
            local txt = _G[self:GetName().."Text"]
            if txt then txt:SetText(tostring(value)) end
        end)
        ...
        -- Durée d'expiration
        local othersExpireLabel = scrollChild:CreateFontString(nil, "OVERLAY", "GameFontNormal")
        othersExpireLabel:SetPoint("TOPLEFT", scrollChild, "TOPLEFT", 20, yOffset)
        othersExpireLabel:SetText(L["SET_EXPIRE_TIME"])

        yOffset = yOffset - 35
        local othersExpireSlider = CreateFrame("Slider", "NMLOthersExpireSlider", scrollChild, "OptionsSliderTemplate")
        othersExpireSlider:SetPoint("TOPLEFT", scrollChild, "TOPLEFT", 30, yOffset)
        othersExpireSlider:SetWidth(200)
        othersExpireSlider:SetMinMaxValues(5, 120)
        othersExpireSlider:SetValueStep(5)
        othersExpireSlider:SetValue(NoMercyListDB.settings.nearbyOthers.expireTime or 30)

        local lowText = _G[othersExpireSlider:GetName().."Low"]
        local highText = _G[othersExpireSlider:GetName().."High"]
        local valueText = _G[othersExpireSlider:GetName().."Text"]
        if lowText then lowText:SetText("5s") end
        if highText then highText:SetText("120s") end
        if valueText then valueText:SetText(tostring(math.floor(NoMercyListDB.settings.nearbyOthers.expireTime or 30)).."s") end

        othersExpireSlider:SetScript("OnValueChanged", function(self, value)
            value = math.floor(value)
            NoMercyListDB.settings.nearbyOthers.expireTime = value
            local txt = _G[self:GetName().."Text"]
            if txt then txt:SetText(tostring(value).."s") end
        end)
        ...
        -- ====================================================================
        -- SECTION 8 : BOUTON MINIMAP
        -- ====================================================================
        MakeSectionTitle(L["SET_SEC_MINIMAP"], yOffset)
        yOffset = yOffset - 35

        local minimapCb = CreateFrame("CheckButton", "NMLMinimapCheckbox", scrollChild, "UICheckButtonTemplate")
        minimapCb:SetPoint("TOPLEFT", scrollChild, "TOPLEFT", 20, yOffset)
        minimapCb.text = minimapCb:CreateFontString(nil, "OVERLAY", "GameFontNormal")
        minimapCb.text:SetPoint("LEFT", minimapCb, "RIGHT", 5, 0)
        minimapCb.text:SetText(L["SET_MINIMAP_SHOW"])
        minimapCb:SetChecked(NoMercyListDB.settings.minimapEnabled)
        minimapCb:SetScript("OnClick", function(self)
            NoMercyListDB.settings.minimapEnabled = self:GetChecked()
            if minimapBtn then
                if NoMercyListDB.settings.minimapEnabled then
                    minimapBtn:Show()
                else
        ...
        resetBtn:SetScript("OnClick", function()
            ...
            NoMercyListDB.settings.nearby.showOthers = true
            if nearbyFrame then
                nearbyFrame:SetScale(1.0)
                if nearbyFrame.bg then
                    nearbyFrame.bg:SetColorTexture(0, 0, 0, 0.8)
                end
            end
            NoMercyListDB.settings.nearbyOthers.maxEntries = 10
            NoMercyListDB.settings.nearbyOthers.expireTime = 30
            ...
            print("|cff00ff00[No Mercy List]|r " .. L["MSG_RESET_OK"])
        end)
...
SlashCmdList["NOMERCYLIST"] = function(msg)
    ...
    elseif cmd:sub(1,4) == "add " then
        ...
        if ok then
            NoMercyList_Refresh()
            print("|cff00ff00[No Mercy List]|r " .. result .. L["MSG_ADDED_MANUAL"])
        elseif result == "doublon" then
            print("|cffff4444[No Mercy List]|r " .. L["MSG_ALREADY_LIST2"])
        else
            print("|cffff4444[No Mercy List]|r " .. L["MSG_INVALID"])
        end

    elseif cmd == "help" then
        print("|cff00ff00[No Mercy List]|r " .. L["HELP_COMMANDS"])
        print(L["HELP_SHOW"])
        print(L["HELP_NEARBY"])
        print(L["HELP_HISTORY"])
        print(L["HELP_SETTINGS"])
        print(L["HELP_ADD"])
        print(L["HELP_ADD_NAME"])
        print(L["HELP_DEBUG"])
        print(L["HELP_TEST"])
        print(L["HELP_HELP"])
    else
